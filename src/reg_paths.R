...
plot_graph <-
    function(estim, layout, mode = "undirected", unique = 0, 
             par = TRUE, rounding = 3, save = NULL, retG = FALSE, ...) {
        # Function for convenience graph plotting of `huge` objects
        #
        # If `unique` is > 0, plot only graph lambda[`unique`].
        # If `save` is defined, a list containing the following elements 
        #   must be passed: 
        #   c(str: "filename", int: dim_x, int: dim_y)
        #   A figure of dimensions dim_x*dim_y in cm in will be saved 
        #   under "figures/filename.png".
        # If `retG` is defined, the function returns the plotted igraph objects. 
        
        lambdas <- estim$lambda
        n <- length(lambdas)
        ...
        if (!is.null(save)) {
            ...
            png(paste0("figures/", filename, ".png"), res=800, pointsize=10, 
                width=dim_x, height=dim_y, units="cm")
        }
        ...
        if (unique) {
            graph <- graph_from_adjacency_matrix(estim$path[[unique]], mode = mode)
            plot(graph,
                 layout = layout,
                 main = paste("Lambda", round(estim$lambda[unique], rounding)),
                 ...)
            ...
            graphs <- list()
            for (i in seq(n)) {
                graph <- graph_from_adjacency_matrix(estim$path[[i]], mode = mode)
                plot(graph,
                     layout = layout,
                     main = paste("Lambda", round(estim$lambda[i], rounding)),
                     ...)
                graphs[[i]] <- graph
            } 
